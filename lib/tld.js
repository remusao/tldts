...
tld.prototype.getDomain = function getDomain(host) {
  ...
  return tld.getFromPattern(host, rule.getPattern());
};

tld.prototype.getDomainFromPattern = function getDomainFromPattern(host, patterns){
  var domain, hostTld;
  ...
  // Existing rules for a TLD
  if (patterns[hostTld]){
    domain = tld.getFromPattern(host, patterns[hostTld]);
  }
  // Try dynamic rule for unknown TLD
  else {
    var rule = new Rule({
      "firstLevel": hostTld,
      "source": hostTld
    ...
    domain = tld.getFromPattern(host, rule.getPattern());
  }

  return domain;
};

tld.getFromPattern = function getFromPattern (lookup, pattern){
  var found = null;

  lookup.replace(new RegExp(pattern), function(m, d){
    found = d;
  });
