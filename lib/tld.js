...
tld.getExceptionXld = function(rule){
  return (rule.secondLevel || '') + '.' + rule.firstLevel;
};

tld.getCompleteXldPattern = function (rule, extractor){
  return (rule.wildcard ? '\\.[^\\.]+' : '') +
    tld[extractor || 'getCompleteXld'](rule).replace('.', '\\.');
};
...
tld.prototype.getDomain = function (host) {
  ...
  pattern = (rule.normal.exception === true)
    ? '('+ tld.getCompleteXldPattern(rule.normal, 'getExceptionXld') +')$'
    : '([^\.]+'+(tld.getCompleteXldPattern(rule.normal))+')$';
