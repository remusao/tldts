...
/**
 * Checks if the tld or sld exists in the actual version of the database
 *
 * @api
 * @param {string} tld
 * @return {boolean}
 */
tld.prototype.domainExists = function domainExists(xld){
  var hostTld, rules;

  xld = (xld+'').trim().toLocaleLowerCase();
  ...
  return this.getRulesForTld(hostTld).some(function(rule){
    return '.'+xld === rule.getNormalXld();
  });
};
